...
const parseNumberFromBuffer = (buffer, start, end) => {
  ...
  for (let i = start; i < end; i++) {
    ...
    const isDigit =
      currentCharCode >= CharCodes.ZERO && currentCharCode <= CharCodes.NINE;
  ...
  const processData = (lineStart, lineEnd) => {
    ...
    const processField = (fieldEnd) => {
      ...
      if (colIndex === spendIndex) {
        currentSpend = parseNumberFromBuffer(buffer, fieldStart, fieldEnd);
        ...
        if (!dateHashToString.has(currentDateHash)) {
          const dateStr = buffer.toString("utf8", fieldStart, fieldEnd);
