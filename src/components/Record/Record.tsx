...
export function Record({record} : {record : RecordType}){
    ...
    return (
        <li className={styles.record}>
            <div className={`${styles.record__info} ${record.isProccessed && styles.record_pointer}`} onClick={record.isProccessed ? () => (setModalState(true)) : () => {return}}>
                ...
                <span>{new Intl.DateTimeFormat('ru-RU').format(new Date(record.date))}</span>
                <div className={`${styles.data} ${!record.isProccessed && styles.not_active}`}>
            ...
            <button className={styles.trash} onClick={() => {deleteRecord(record.id)}}>
                <img src="../../../img/trash-icon.svg" className={styles.icon} />
            </button>
            {
                record.data &&
                <>
                    <Modal isOpen={isModalOpen} setModalState={setModalState} data={record.data}/>
                </>
            }
        </li>
    )
